...
{
  "is_approved": true,
  "vulnerabilities": ["List of critical violations or unresolved linter errors"],
  "suggestions": ["List of suggestions for improvement"]
}
```

Ensure `vulnerabilities` and `suggestions` are empty lists `[]` if there are none. Set `is_approved` to `true` ONLY if there are no vulnerabilities, otherwise set it to `false`.
