...
def test_raw_quote_valid():
    quote = RawQuote(
        date=date(2023, 1, 1), open=100.0, high=110.0, low=90.0, close=105.0, volume=1000
    )
...
def test_raw_quote_invalid_high_less_than_low():
    with pytest.raises(
        ...
        RawQuote(date=date(2023, 1, 1), open=100.0, high=90.0, low=110.0, close=105.0, volume=1000)


def test_raw_quote_invalid_high_less_than_open():
    with pytest.raises(
        ValidationError, match="High price must be greater than or equal to open price."
    ):
...
def test_raw_quote_invalid_low_greater_than_close():
    with pytest.raises(
        ValidationError, match="Low price must be less than or equal to close price."
    ):
        RawQuote(date=date(2023, 1, 1), open=110.0, high=115.0, low=106.0, close=105.0, volume=1000)
