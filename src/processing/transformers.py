...
def transform_quotes(quotes: list[RawQuote]) -> pl.DataFrame:
    ...
    df = df.with_columns([
        ...
        (pl.col("date").dt.month() != pl.col("date").dt.month().shift(1)).fill_null(False).alias("is_month_start"),
        (pl.col("date").dt.month() != pl.col("date").dt.month().shift(-1)).fill_null(False).alias("is_month_end"),
    ])
